...
import torch
from src.VARS_similar import GRAPH_MODEL_NAME
import src.VARS_similar as VARS
import subprocess
...
from src.graph_handler_similar import *
from src.llm_handler_qwen3 import solve_problem
import time
...
def main():
    ...
    for i, problem_data in enumerate(problems):
        ...
        expected_answer = problem_data.get("Answer", None)
        if expected_answer is None:
            expected_answer = problem_data.get("answer", None)
        # check if the problem_data have 'options' filed and append them to the problem_text with newline separation and label them as A., B., C., D. etc.
        options = problem_data.get("Options", [])
        if options and isinstance(options, list):
            options_str = "\n".join(
                [f"{chr(65 + idx)}. {opt.strip()}" for idx, opt in enumerate(options)]
            )
