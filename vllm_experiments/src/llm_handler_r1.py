...
def solve_problem(
    ...
    template_prompt = ""
    if template:
        template_prompt = "Solve the math problem and always conclude with:\nTherefore, the final answer is: $\\boxed{answer}$. Where [answer] is just the final numerical answer that solves the problem. Ensure the number is clearly identifiable within the box.\nYou are given a template to solve the problem. Use the given steps if applicable otherwise use them to guide your reasoning and present the solution steps logically to solve this problem:\n"
...
Where [answer] is just the final numerical answer that solves the problem. Ensure the number is clearly identifiable within the box."""
    ...
    if thinking_interven and template_str:
        ...
        current_prompt = f"{template_prompt}\nProblem: {problem_text}.<｜Assistant｜><think>I need to follow the given steps to guide me in reasoning to solve the problem. I will follow each step and modify the steps to make them match the problem and then solve the problem accordingly. The template is. {template_str.replace('\n', ' ')}Strictly following the given steps for guidance, I will now solve the problem starting from the step 1. Using step 1"
        logging.info(
